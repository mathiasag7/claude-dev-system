...
[2026-07-22] — opencrm — Le Step-4a Impact Scan a SOUS-DIMENSIONNÉ une SUPPRESSION/RENAME de champ (reco.product, CRM Product.margin_components/standard_rate, interest.product). Le scan capé a raté deux lecteurs LIVE : un endpoint @frappe.whitelist (eligibility.annotate_recommendations lisant reco.product) et un CLIENT-SCRIPT embarqué dans un .py (crm_products.py lisant CRM Product.standard_rate) — plus la porte funnel + test_crm_deal. Rattrapés seulement par un grep final APRÈS édition. → Pour toute SUPPRESSION/RENAME de champ, grepper le NOM EXACT du champ sur TOUT le code AVANT d'éditer (pas un inventaire capé) : .py logique ET chaînes JS de client-scripts embarquées dans des .py ET endpoints whitelistés ET frontend. Un champ retiré casse silencieusement ses lecteurs (get_value renvoie None sans erreur, §1.9). → Step 4a renforcé.
```
